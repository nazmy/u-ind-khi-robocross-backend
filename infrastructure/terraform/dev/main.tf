...
//temp for prod using dev subscription
resource "azurerm_linux_web_app" "khirobocross-dev-prod" {
  name                = "${var.resource_prefix}-dev-prod-api"
  resource_group_name = azurerm_resource_group.backend_rg.name
  location            = azurerm_service_plan.khirobocross.location
  service_plan_id     = azurerm_service_plan.khirobocross.id
  ...
  app_settings = {
    "AZURE_MONGDB_DATABASENAME"      = var.azure_devprod_mongodb_databasename
    "ASPNETCORE_ENVIRONMENT"         = var.aspnetcore_environment
    "DOTNET_ENVIRONMENT"             = var.aspnetcore_environment
    "APPINSIGHTS_INSTRUMENTATIONKEY" = var.appservice_appinsight_instrumentation_key
  }
  ...
  connection_string {
    name  = "COSMOS_DB_CONNECTION_STRING"
    value = var.cosmo_devprod_db_connection_string
    type  = "Custom"
  }
